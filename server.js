...
function freshNight(day){ return {day,mafiaTargetId:null,doctorTargetId:null,resolved:false,result:null}; }
...
function publicRoom(r){ return {code:r.code,expiresAt:r.expiresAt,day:r.day,status:r.status,voteStatus:r.voteStatus,playerCount:r.players.length,aliveCount:r.players.filter(p=>p.alive).length,rolesAssigned:r.rolesAssigned,roleLocked:!!r.roleLocked}; }
...
function teacherState(r, showRoles=false){
  ...
  return {
    ...
    settings:r.settings,
    players:r.players.map(p=>({id:p.id,name:p.name,role:showRoles?p.role:null,alive:p.alive,joinedAt:p.joinedAt,connected:connected(p),hasVoted:!!r.currentVotes[p.id]})),
    ...
    nightAction:{...night,mafiaTargetName:playerName(r,night.mafiaTargetId),doctorTargetName:playerName(r,night.doctorTargetId)},
...
function studentState(r,p){
  ...
  return {
    ...publicRoom(r),
    player:{id:p.id,name:p.name,alive:p.alive,role:roleVisible?p.role:null,roleLabel:roleVisible?roleLabel(p.role):null,fellowMafia:fellow,hasVoted:!!r.currentVotes[p.id],votedTargetId:r.currentVotes[p.id]||null},
...
async function api(req,res,u){
  ...
  if(method==='POST'&&action==='vote/start'){
    if(!isTeacher)return json(res,403,{error:'교사 권한이 없습니다.'}); if(!r.rolesAssigned)return json(res,400,{error:'먼저 역할을 배정해 주세요.'}); if(r.voteStatus==='open')return json(res,400,{error:'이미 투표가 진행 중입니다.'}); r.currentVotes={};r.voteStatus='open';saveRooms();return json(res,200,teacherState(r,false));
  }
  ...
  if(method==='POST'&&action==='eliminate'){
    if(!isTeacher)return json(res,403,{error:'교사 권한이 없습니다.'}); if(r.voteStatus!=='ended')return json(res,400,{error:'투표를 종료한 뒤 탈락 처리해 주세요.'}); if(r.voteHistory.some(h=>h.day===r.day))return json(res,400,{error:'이번 일차의 탈락 처리가 이미 완료되었습니다.'}); const p=r.players.find(x=>x.id===String(b.playerId||'')); if(!p||!p.alive)return json(res,400,{error:'탈락 처리할 수 없는 학생입니다.'}); p.alive=false; r.voteHistory.push({day:r.day,endedAt:now(),eliminatedPlayerId:p.id,eliminatedName:p.name,tally:tally(r),votes:{...r.currentVotes}}); if(r.nightAction?.mafiaTargetId===p.id)r.nightAction.mafiaTargetId=null;if(r.nightAction?.doctorTargetId===p.id)r.nightAction.doctorTargetId=null; saveRooms();return json(res,200,teacherState(r,false));
  }
  if(method==='POST'&&action==='night/set'){
    if(!isTeacher)return json(res,403,{error:'교사 권한이 없습니다.'}); if(!r.rolesAssigned)return json(res,400,{error:'먼저 역할을 배정해 주세요.'}); normalizeRoom(r); if(r.nightAction.resolved)return json(res,400,{error:'이번 일차의 밤 결과가 이미 적용되었습니다.'});
    const mafiaTargetId=b.mafiaTargetId?String(b.mafiaTargetId):null; const doctorTargetId=b.doctorTargetId?String(b.doctorTargetId):null;
    if(mafiaTargetId&&!r.players.some(p=>p.id===mafiaTargetId&&p.alive))return json(res,400,{error:'마피아 지목 대상은 생존 학생만 선택할 수 있습니다.'});
    if(doctorTargetId&&!r.players.some(p=>p.id===doctorTargetId&&p.alive))return json(res,400,{error:'의사 보호 대상은 생존 학생만 선택할 수 있습니다.'});
    r.nightAction.mafiaTargetId=mafiaTargetId; r.nightAction.doctorTargetId=doctorTargetId; saveRooms(); return json(res,200,{ok:true,nightAction:r.nightAction});
  }
  if(method==='POST'&&action==='night/resolve'){
    if(!isTeacher)return json(res,403,{error:'교사 권한이 없습니다.'}); if(!r.rolesAssigned)return json(res,400,{error:'먼저 역할을 배정해 주세요.'}); normalizeRoom(r); if(r.nightAction.resolved)return json(res,400,{error:'이번 일차의 밤 결과가 이미 적용되었습니다.'});
    const mafiaAlive=r.players.some(p=>p.alive&&p.role==='mafia');
    const target=r.players.find(p=>p.id===r.nightAction.mafiaTargetId&&p.alive);
    const doctorTarget=r.players.find(p=>p.id===r.nightAction.doctorTargetId&&p.alive);
    if(mafiaAlive&&!target)return json(res,400,{error:'마피아가 지목한 학생을 먼저 체크해 주세요.'});
    ...
    const result={day:r.day,resolvedAt:now(),mafiaTargetId:target?.id||null,mafiaTargetName:target?.name||null,doctorTargetId:doctorTarget?.id||null,doctorTargetName:doctorTarget?.name||null,protected:protectedSuccess,eliminatedPlayerId:eliminated?.id||null,eliminatedName:eliminated?.name||null};
    r.nightAction.resolved=true;r.nightAction.result=result;r.nightHistory.push(result);saveRooms();return json(res,200,{ok:true,result});
  }
  if(method==='POST'&&action==='day/next'){
    if(!isTeacher)return json(res,403,{error:'교사 권한이 없습니다.'}); if(r.voteStatus==='open')return json(res,400,{error:'진행 중인 투표를 먼저 종료해 주세요.'}); if(r.nightAction?.mafiaTargetId&&!r.nightAction.resolved)return json(res,400,{error:'마피아 지목이 저장되어 있습니다. 밤 결과를 적용하거나 선택을 지운 뒤 다음 일차로 넘어가 주세요.'});
